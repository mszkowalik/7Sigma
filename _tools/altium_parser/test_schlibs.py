...
class TestSchLibs(TestCase):
    ...
    def test_schlib_designator_name(self, lib):
        for pin in lib.pins:
            self.assertRegex(pin.designator, "^(EP[1-9]|MH\\d?|\\d+|[A-Z]+\\d+)$")
    ...
    def test_power_in_should_be_input(self, lib):
        for pin in lib.pins:
            ...
            if (name.find('vcc') != -1 or name.find('vdd') != -1 or name.find('vin') != -1) and name.find('out') == -1:
                self.assertEqual(Pin_ElectricalType.Input, pin.electrical_type, "In name: {}, {}".format(name, pin.designator))
    ...
    def test_designator_match_prefix(self, lib):
        ...
        self.assertEqual(correct_designator[prefix] + '?', lib.designator.text)
